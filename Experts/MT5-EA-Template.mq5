...
#property description   "Jules EA Implementation - Clean Slate (Aggressive M1 Mod)"
...
//-INPUTS-//
input group "Strateji Ayarlari (Agresif Mod)"
input double InpStopLossUSD    = 15.0;   // Islem Basina Max Zarar Kes (USD - Hard SL)
input double InpLotSize        = 0.01;   // Islem Hacmi (Lot)
input int    InpMAHours        = 70;     // Hareketli Ortalama Periyodu (MA Filter)
input int    InpTrendPeriod    = 5;      // Trend Teyidi Mum Sayisi (M1 icin 5 yeterli)
input int    InpDelaySeconds   = 0;      // Maksimum Giris Gecikmesi (Saniye - Agresif=0)
...
input double InpMaxSpreadPoints= 50;     // Max Spread (Puan, 0=Devre Disi) (Agresif=50)
input int    InpMaxOpenPositions = 356;  // Global Maksimum Acik Pozisyon (Default: 356)
input int    InpMaxTradeHours  = 0;      // Zaman Asimi (Saat) - 0 = Kapali

input group "Zaman Ayarlari (Gate)"
input bool   InpEnableSundayGate = false;// Pazar Gate Aktif Et (Agresif Modda Kapali)
...
input string InpComment        = "JulesEA_Aggressive"; // Emir Yorumu
...
void ExecuteTrades(int direction)
{
    ...
    for(int i=0; i<count; i++)
    {
        ...
        // Calculate Hard SL based on exact USD Loss
        double point       = SymbolInfoDouble(Symbol(), SYMBOL_POINT);
        ...
        // Formula: slDistancePoints = lossUSD / (Lot * (TickValue / (TickSize / Point)))
        if (tickValue > 0 && tickSize > 0 && point > 0 && InpStopLossUSD > 0 && InpLotSize > 0)
        {
             double valuePerPoint = tickValue / (tickSize / point);
             slDistancePoints = InpStopLossUSD / (InpLotSize * valuePerPoint);
        }
        ...
        if (direction == 1) // BUY
        {
            ...
            {
                sl = NormalizeDouble(ask - (slDistancePoints * point), _Digits);
            }
        ...
        else if (direction == -1) // SELL
        {
            ...
            if (slDistancePoints > 0)
            {
                sl = NormalizeDouble(bid + (slDistancePoints * point), _Digits);
            }
...
int OnInit()
{
    ...
    // Task 5: Initialize MA Handle (Current Period, SMA, Close - Agresif Mod)
    ExtHandleMA = iMA(Symbol(), PERIOD_CURRENT, InpMAHours, 0, MODE_SMA, PRICE_CLOSE);
...
void OnTick()
{
    ...
    if (IsNewBar())
    {
        ...
        if (CheckReversal(rates, allowedDirection))
        {
            ...
            ExtPendingDelay = (int)(MathRand() % (InpDelaySeconds + 1)); // Delay is 0 in aggressive mode
